...
def postgres_schema():
    """
    Create an isolated schema for this integration-test module.

    This prevents records from the normal application database from
    contaminating analytics integration tests.
    """
...
@pytest.fixture
def db_session(postgres_schema):
...
def test_analytics_endpoint_with_postgresql(
    ...
    assert (
        distribution["Excellent"]["percentage"]
        == pytest.approx(100.0)
    )
    ...
    assert (
        distribution["Need Improvements"]["count"]
        == 0
    )
    ...
    assert (
        attention["poor_feedback_count"]
        == 0
    )

    assert (
        attention["need_improvements_count"]
        == 0
    )

    assert (
        attention["low_confidence_count"]
        == 0
    )
